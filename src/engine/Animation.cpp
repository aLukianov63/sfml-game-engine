...
Animation& Animation::addFrameColumn(int number_x,int number_y,int column)
{
...
Animation& Animation::addFrameLine(int number_x,int number_y,int line)
{
	const sf::Vector2u size = _texture->getSize();
	const float delta_x = size.x / float(number_x);
	const float delta_y = size.y / float(number_y);
...
auto Animation::addFrameLineFromBack(int number_x, int number_y, int line) -> Animation&
{
	const sf::Vector2u size = _texture->getSize();
	const float delta_x = size.x / float(number_x);
	const float delta_y = size.y / float(number_y);

	for(int i = number_x - 1; i > 0; --i)
